...
# Elbow 
def find_optimal_k(X: np.ndarray, k_range: range, config: Dict[str, Any]) -> Optional[int]:
    print("Finding Optimal K...")
    if len(X) < max(k_range): return None
    inertias = [
...
def main(config: Dict[str, Any]):
   ...
   #results
    ...
    for window_size in config["window_size_range"]:
        ...
        optimal_k = find_optimal_k(X, config["k_range_to_test"], config)

        if optimal_k:
            print(f"--- Performing Final Clustering with k={optimal_k} ---")
            final_km = TimeSeriesKMeans(
                n_clusters=optimal_k, metric="dtw", 
